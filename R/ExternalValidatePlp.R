...
externalValidatePlp <- function(plpResult,
  ...
  for(i in 1:length(validationSchemaCdm)){
    ...
    newData <- CauseOfDeathImputation::similarPlpData(plpModel= plpResult$model, createCohorts = F, 
    ...
    if(sum(newData$population$outcomeCount>0)<5){
      ...
      if(sum(newData$population$outcomeCount>0) > 0){
      results[[i]] <- CauseOfDeathImputation::applyModel(population=newData$population, plpData = newData$plpData, 
                                                         calculatePerformance = T, plpModel = plpResult$model)
      }else if (sum(newData$population$outcomeCount>0) == 0){
        results[[i]] <- CauseOfDeathImputation::applyModel(population=newData$population, plpData = newData$plpData, 
                                   calculatePerformance = F, plpModel = plpResult$model)
      }else{}
      
      cohorts <- newData$plpData$cohorts
      cohorts <- cohorts %>% select(rowId, subjectId, cohortId, cohortStartDate)
      cohorts$cohortStartDate <- as.numeric(format(cohorts$cohortStartDate, '%Y'))
  ...
  summary <- reshape2::dcast(summary, Database ~ Metric, value.var="Value" )
    ...
    summary <- "Validation result of model without labels"
  }
